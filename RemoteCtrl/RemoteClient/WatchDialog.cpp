...
{	// 800 450    1920 1080    16:9   (本电脑分辨率：2560 1440)
	...
	m_picture.ScreenToClient(&point);   // 转换为客户区域坐标（相对picture空间左上角的坐标）
	// TRACE("x:%d, y:%d\r\n", point.x, point.y);
	// 本地坐标到远程坐标
...
void CWatchDialog::OnLButtonDown(UINT nFlags, CPoint point)
{
	if ((m_nObjWidth != -1) && (m_nObjHeight != -1)) {
		...
		CClientController::getInstance()->SendCommandPacket(GetSafeHwnd(), 5, true, (BYTE*) & event, sizeof(event));  // 是sizeof(MOUSEEV)还是sizeof(event)？ 都一样吧
